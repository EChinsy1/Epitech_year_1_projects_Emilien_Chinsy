...
void swap(file_t *a, file_t *b, int *swapped)
{
    ...
    char *lowcase_name = my_strlowcase(my_strdup(a->name));
    char *lowcase_name_next = my_strlowcase(my_strdup(a->next->name));

    if (my_strcmp(lowcase_name, lowcase_name_next) > 0) {
        a->name = b->name;
        a->path = b->path;
        b->name = temp_name;
        b->path = temp_path;
...
void sort_manager(char_list_t *flags, file_t **items)
{
