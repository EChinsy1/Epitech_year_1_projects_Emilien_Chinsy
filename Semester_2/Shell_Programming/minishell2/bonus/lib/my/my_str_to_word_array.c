...
static char *find_in_env(char *value, args_t *args)
{
    int pos = 0;
...
static char *find_in_history(char *value, args_t *args)
{
    int pos = 0;

    for (int i = 0; args->history[i]; ++i) {
        if (my_strncmp(&value[1], args->history[i], my_strlen(&value[1])) == 0) {
            free(value);
...
char **remove_extras(char **word_array, int words, args_t *args)
{
...
char **my_str_to_word_array(char *str, char *delim, args_t *args)
{
    ...
    return (remove_extras(word_array, words, args));
}
