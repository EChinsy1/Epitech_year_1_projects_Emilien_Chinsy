...
int error_handling(args_t *args)
{
    ...
    if (my_str_isprintable(args->word_array[1]) != 1) {
        my_putstr(2,
...
int setenv_func(args_t *args)
{
    size_t size = 100;
    int where = 0;
    int errors = error_handling(args);

    if (errors != 0)
        return (errors);
    if (errors == -269)
        return (0);
    if (args->word_array[2] != NULL && args->word_array[3] != NULL) {
        my_putstr(2, "setenv: Too many arguments\n");
        free_word_array(args->word_array);
        return (1);
    }
    args->env = modify_env(args, args->word_array[1], args->word_array[2]);
    free_word_array(args->word_array);
    return (0);
}
