...
static char *try_with_normal_path(char **command_array)
{
    char *test_path = create_path("/bin/", command_array[0]);
...
char *get_directory_exec(args_t *args, char **command_array)
{
    ...
    if (args->env[i] == NULL)
        return (try_with_normal_path(command_array));
    if (my_strlen(args->env[i]) <= PATH_LEN)
        return (NULL);
